...
| 7 | Example: C = 14,796; A and B differ by about 39 µs; the fraction added is 0.164 pulse. Without interpolation one pulse is ≈ 0.007 % here. | `prover_demo_data.py` (CHRONO_*) | 1/14796 = 0.0068 %. Derived, not a site value. |
...
| 13 | [+] Guideline: 0 to +5 % above the calculated value, i.e. 68 to 71.4 psig; vent the excess or charge from the supply bottle. | manual §3.2 p.32 note 1 and first paragraph | 71.4 = PLENUM_MAX. |
...
| 15 | Both volumes are determined by water draw; the manual gives their ratio per size (24": 0.992369). | manual §2.3 p.23; §4.2.1 p.37; Table 1-2 p.8 | |
...
| 22 | The Invar linear coefficient is 7.5 times smaller than the tube's linear coefficient. | derived: 0.0000108 / 0.00000144 = 7.5 | |
...
| 24 | Example: Tp = 30 °C → tube term 1.000324; Td = 15 °C → rod term 1; CTSp = 1.000324 (raises the prover volume by ≈ 3 parts in 10,000). In the field the rod term is not always 1. | `prover_demo_data.py` (CTSP_*) | Owner decision 1. |
