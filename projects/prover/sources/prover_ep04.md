...
| 4 | Proving (the pass): the flag on the piston trips the volume switches; the signals go to the computer instantly; between them the piston displaces the base volume while meter pulses are counted. | manual §3.1 step 3 p.27; §1.3 pp.2–3 | Pulse counting between the switches: episodes 1–2 (projects/prover/sources/prover_ep01.md #5, #7). How the computer times the pulses is left for episode 5. |
| 5 | In our example the pass takes about 3.5 s. | `projects/prover/prover_demo_data.py` (PASS_TIME = 3.547 s) | Spoken as "about three and a half seconds". |
...
| 9 | In our data the cycle repeats 3 times per run. | `projects/prover/prover_demo_data.py` (PASSES_PER_RUN = 3) | Series terminology: pass = الشوط, run = الجولة. |
