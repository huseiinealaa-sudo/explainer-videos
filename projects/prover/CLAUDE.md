# Project: Daniel Compact Prover (7 episodes)

Series-specific rules. The general rules in the root `CLAUDE.md` also apply; where they differ, this file wins for this project.
Published videos: `output/prover_ep01_principle.mp4` … `output/prover_ep07_audit.mp4` and `output/prover_full_series.mp4` (they stay in the flat `output/` folder; do not move or re-render them).
This series was produced before the root Fast workflow existed: it has no `sources/prover_source.md`; its sources are `projects/prover/sources/prover_ep01.md` … `prover_ep07.md`.
...
- All numbers shown in any episode MUST come from projects/prover/prover_demo_data.py. Never type derived values by hand.
...
## projects/prover/prover_demo_data.py must compute
...
## Episode 5 — owner decisions (2026-09-25; details in projects/prover/sources/prover_ep05.md)
...
- Each episode 3–4 minutes. Files: projects/prover/prover_epNN_<topic>.py and output/prover_epNN_<topic>.mp4.
...
## Research and verification (series specifics)
...
- Save sources for each episode in projects/prover/sources/prover_epNN.md: claim → source URL → short note.
